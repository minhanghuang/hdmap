...
bool Box2d::IsPointIn(const Vec2d& point) const {
  ...
  return dx <= half_length_ + 1e-10 && dy <= half_width_ + 1e-10;
}
...
bool Box2d::IsPointOnBoundary(const Vec2d& point) const {
  ...
  const double dx = std::abs(x0 * cos_heading_ + y0 * sin_heading_);
  const double dy = std::abs(x0 * sin_heading_ - y0 * cos_heading_);
  return (std::abs(dx - half_length_) <= 1e-10 && dy <= half_width_ + 1e-10) ||
         (std::abs(dy - half_width_) <= 1e-10 && dx <= half_length_ + 1e-10);
}
      ...
      case 2:
        return (x1 > x2) ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                         ...
                         : PtSegDistance(box_x, box_y, x1, y1, x2, y2,
                                         line_segment.length());
      case -1:
        return Vec2d::CrossProd({x1, y1}, {x2, y2}, {box_x, -box_y}) >= 0.0
                   ? 0.0
                   : PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                   line_segment.length());
      case -4:
        return Vec2d::CrossProd({x1, y1}, {x2, y2}, {box_x, -box_y}) <= 0.0
                   ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                   line_segment.length())
                   : (Vec2d::CrossProd({x1, y1}, {x2, y2}, {-box_x, box_y}) <=
                              0.0
                          ? 0.0
                          : PtSegDistance(-box_x, box_y, x1, y1, x2, y2,
                                          line_segment.length()));
...
void Box2d::RotateFromCenter(const double rotate_angle) {
  heading_ = math::NormalizeAngle(heading_ + rotate_angle);
