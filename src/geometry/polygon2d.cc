#include "opendrive-engine/geometry/polygon2d.h"
...
bool Polygon2d::IsPointIn(const Vec2d& point) const {
  ...
  for (int i = 0; i < num_points_; ++i) {
    if ((points_[i].y() > point.y()) != (points_[j].y() > point.y())) {
      const double side = Vec2d::CrossProd(point, points_[i], points_[j]);
...
bool Polygon2d::HasOverlap(const Polygon2d& polygon) const {
  ...
  return DistanceTo(polygon) <= 1e-10;
}
...
bool Polygon2d::Contains(const LineSegment2d& line_segment) const {
  ...
  if (!is_convex_) {
    ...
    return total_length >= line_segment.length() - 1e-10;
  }
  return true;
}

bool Polygon2d::Contains(const Polygon2d& polygon) const {
  if (area_ < polygon.area() - 1e-10) {
    return false;
  }
  if (!IsPointIn(polygon.points()[0])) {
    return false;
...
void Polygon2d::BuildFromPoints() {
  ...
  for (int i = 1; i < num_points_; ++i) {
    area_ += Vec2d::CrossProd(points_[0], points_[i - 1], points_[i]);
  }
  if (area_ < 0) {
    area_ = -area_;
  ...
  is_convex_ = true;
  for (int i = 0; i < num_points_; ++i) {
    if (Vec2d::CrossProd(points_[Prev(i)], points_[i], points_[Next(i)]) <=
        -1e-10) {
      is_convex_ = false;
      break;
    }
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d>& points,
  ...
  for (int i = 0; i < n + n; ++i) {
    ...
    while (count > last_count &&
           Vec2d::CrossProd(points[results[count - 2]],
                            points[results[count - 1]], pt) <= 1e-10) {
      results.pop_back();
      --count;
    }
...
bool Polygon2d::ClipConvexHull(const LineSegment2d& line_segment,
  ...
  for (size_t i = 0; i < n; ++i) {
    prod[i] = Vec2d::CrossProd(line_segment.start(), line_segment.end(),
                               (*points)[i]);
    if (std::abs(prod[i]) <= 1e-10) {
      side[i] = 0;
    } else {
      side[i] = ((prod[i] < 0) ? -1 : 1);
    }
...
bool Polygon2d::GetOverlap(const LineSegment2d& line_segment,
  ...
  return min_proj <= max_proj + 1e-10;
}
...
std::vector<LineSegment2d> Polygon2d::GetAllOverlaps(
  ...
  for (size_t i = 0; i + 1 < projections.size(); ++i) {
    ...
    if (overlaps.empty() || start_proj > overlaps.back().second + 1e-10) {
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {
  ...
  for (int i = 0; i < num_points_; ++i) {
    ...
    const double diff = math::WrapAngle(end_angle - start_angle);
    if (diff <= 1e-10) {
