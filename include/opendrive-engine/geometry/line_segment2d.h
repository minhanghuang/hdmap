#ifndef OPENDRIVE_ENGINE_GEOMETRY_LINE_SEGMENT2D_H_
#define OPENDRIVE_ENGINE_GEOMETRY_LINE_SEGMENT2D_H_
...
class LineSegment2d {
 public:
   ...
   */
  LineSegment2d(const geometry::Vec2d& start, const geometry::Vec2d& end);

  /**
   * @brief Get the start point.
   * @return The start point of the line segment.
   */
  const geometry::Vec2d& start() const { return start_; }

  /**
   * @brief Get the end point.
   * @return The end point of the line segment.
   */
  const geometry::Vec2d& end() const { return end_; }

  /**
   * @brief Get the unit direction from the start point to the end point.
   * @return The start point of the line segment.
   */
  const geometry::Vec2d& unit_direction() const { return unit_direction_; }

  /**
   * @brief Get the center of the line segment.
   * @return The center of the line segment.
   */
  geometry::Vec2d center() const { return (start_ + end_) / 2.0; }
   ...
   */
  geometry::Vec2d rotate(const double angle);
   ...
   */
  double DistanceTo(const geometry::Vec2d& point) const;
   ...
   *         to the input point.
   */
  double DistanceTo(const geometry::Vec2d& point,
   ...
   */
  double DistanceSquareTo(const geometry::Vec2d& point) const;
   ...
   *         to the input point.
   */
  double DistanceSquareTo(const geometry::Vec2d& point,
                          geometry::Vec2d* const nearest_pt) const;
   ...
   */
  bool IsPointIn(const geometry::Vec2d& point) const;
   ...
   */
  double ProjectOntoUnit(const geometry::Vec2d& point) const;
   ...
   */
  double ProductOntoUnit(const geometry::Vec2d& point) const;
   ...
   */
  double GetPerpendicularFoot(const geometry::Vec2d& point,
                              geometry::Vec2d* const foot_point) const;

 private:
  geometry::Vec2d start_;
  geometry::Vec2d end_;
  geometry::Vec2d unit_direction_;
  double heading_;
  double length_;
};
...
#endif  // OPENDRIVE_ENGINE_GEOMETRY_LINE_SEGMENT2D_H_
