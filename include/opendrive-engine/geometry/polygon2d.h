...
class Polygon2d {
 public:
   ...
   */
  explicit Polygon2d(const std::vector<geometry::Vec2d>& points);

  /**
   * @brief Get the vertices of the polygon.
   * @return The vertices of the polygon.
   */
  const std::vector<geometry::Vec2d>& points() const { return points_; }
   ...
   */
  double DistanceToBoundary(const geometry::Vec2d& point) const;
   ...
   */
  bool IsPointOnBoundary(const geometry::Vec2d& point) const;
   ...
   */
  static bool ComputeConvexHull(const std::vector<geometry::Vec2d>& points,
  ...
  bool GetOverlap(const LineSegment2d& line_segment,
                  geometry::Vec2d* const first,
                  geometry::Vec2d* const last) const;

  /**
   * @brief Get all vertices of the polygon
   * @param All vertices of the polygon
   */
  void GetAllVertices(std::vector<geometry::Vec2d>* const vertices) const;

  /**
   * @brief Get all vertices of the polygon
   */
  std::vector<geometry::Vec2d> GetAllVertices() const;
   ...
   *        projection onto the heading direction.
   */
  void ExtremePoints(const double heading, geometry::Vec2d* const first,
                     geometry::Vec2d* const last) const;
 ...
 protected:
  ...
  static bool ClipConvexHull(const LineSegment2d& line_segment,
                             std::vector<geometry::Vec2d>* const points);
  std::vector<LineSegment2d> line_segments_;
  std::vector<geometry::Vec2d> points_;
  int num_points_;
  bool is_convex_;
  double area_;
  double min_x_;
...
#endif  // OPENDRIVE_ENGINE_GEOMETRY_POLYGON2D_H_
