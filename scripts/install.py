...
class Worker:
    ...
    def install(self):
        ...
        for pro in os.listdir(self.download_path):
            ...
            print(path)
            cmd = "export PKG_CONFIG_PATH={0}/lib/pkgconfig:{0}/share/pkgconfig:$PKG_CONFIG_PATH && cd {1} && cmake -B build -DCMAKE_INSTALL_PREFIX={0} && cmake --build build -j4 && cd build && make install".format(
